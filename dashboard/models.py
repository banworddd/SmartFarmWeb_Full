...
class SensorData(models.Model):
    """
    Модель данных датчиков.

    Эта модель хранит различные показания и измерения, собранные с датчиков устройств на ферме,
    включая температуру, влажность, влажность почвы и другие параметры окружающей среды.

    Атрибуты:
        - device (ForeignKey): Устройство-датчик, с которого получены данные.
        - timestamp (datetime): Дата и время получения данных.
        - temperature (float): Показания температуры в градусах Цельсия.
        - humidity (float): Показания влажности воздуха в процентах.
        - soil_moisture (float): Показания влажности почвы в процентах.
        - light_intensity (float): Показания интенсивности света в люксах.
        - ph_level (float): Показания уровня pH.
        - battery_level (float): Уровень заряда батареи устройства в процентах.
        - additional_data (JSONField): Дополнительные данные датчика в формате JSON.

    Методы:
        - __str__(): Возвращает строковое представление записи данных с указанием устройства и времени.
    """
...
class DeviceStatus(models.Model):
    ...
    Хранит информацию о текущем состоянии устройства, включая его подключение к сети,
    загрузку ресурсов, силу сигнала и другие технические параметры.

    Поля:
        - device (ForeignKey): Устройство, к которому относится данный статус.
        - timestamp (DateTime): Время создания записи о статусе (устанавливается автоматически).
        - online (bool): Флаг, указывающий, находится ли устройство в сети.
        - cpu_usage (float): Загрузка процессора в процентах (0–100).
        - memory_usage (float): Загрузка оперативной памяти в процентах (0–100).
        - disk_usage (float): Загрузка диска в процентах (0–100).
        - signal_strength (float): Сила сигнала устройства в dBm.
        - additional_info (JSON): Дополнительная техническая информация (может включать произвольные данные).

    Метаданные:
        - Упорядочивание по убыванию временной метки (новейшие статусы первыми).
        - Индексация по времени и комбинации device+online для ускорения запросов.

    Методы:
        - __str__(): Возвращает строковое представление статуса устройства в формате "<название устройства> is <онлайн/оффлайн> at <время>".

    Применение:
        Используется для мониторинга и логирования текущего состояния устройств,
        а также для отображения статуса на панели управления фермы.
    """
